...
void IIMain()
{
	for(freq_bin_index = 0;freq_bin_index<freq_bin_NO;freq_bin_index++)
	{
		//freq_bin[freq_bin_index] = 800;
	}
	freq_bin_index = 0;

	search_SVprn1to32 = 30; // start search with PRN x
	//cio << "search PRN" << (search_SVprn1to32-1)%32+1 << endl;	cio << noshowcursor << flush;
...
void change_integration_period(int chn, Irq *p_myIRQ)
{
    ...
    else
    {
        ...
        {
            case CHANGE_TO_2ms:
				cio << "change_integration_period 2ms" << endl;
            ...
            case CHANGE_TO_5ms:
				cio << "change_integration_period 5ms" << endl;
            ...
            case CHANGE_TO_10ms:
				cio << "change_integration_period 10ms" << endl;
                /* Added code phase shift to account for transients JC 17/11/95 */
            ...
            case CHANGE_TO_POINT_ONE_10ms:
				cio << "change_integration_period .1 10ms" << endl;
                /**************************************************************/
                /*                                                            */
...
*/
    ...
    if(bit_counter[chn] >= bit_length) // 20 for GPS and 10 for GLONASS
    {
        data_store[chn] <<= 1;
...
{   
    ...
    switch(sys[chn])
    {
        case GLONASS:
		    ...
		    //cio << bold << "disabled GLONASS, warning tgghtrey\n" << normal << endl;
            carrier_nominal_freq  = carrier_low[L1][chn] - GLN_L1_WORD[channel_num[chn]];
            carrier_nominal_freq += (carrier_top[L1][chn] - GLN_L1_TOP[channel_num[chn]]) * TWO_POWER_32;
...
void steady_state_long_integration_CA_tracking(int chn, Irq *p_myIRQ)
{
    ...
    else
    {
        ...
        {
		    //cio << bold << "disabled GLONASS, warning sdhfghreh\n" << normal << endl;
			...
			//cio << "enter STATE_7 from steady_state_long_integration_CA_tracking" << normal << endl; cio << noshowcursor << flush;
...
void P_code_tracking(int chn, Irq *p_myIRQ)
{
    ...
    if(tracking_state[chn] == STATE_7)
    {
        ...
        read_correlator_totals(chn,P_L2,NORM);
        P_code_timeout[chn]++;
        if((shift_p[chn] == FIRST) && (p_chips_added[chn] == 0))
        {
