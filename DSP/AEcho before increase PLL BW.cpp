...
void IIMain()
{
	...
	search_SVprn1to32 = 1; // start search with PRN x
	

	int temp = 0;
    cio << init;
    cio.At(Point(10, 0));
	sprintf(OPstring,"Peiqing GPS/GLONASS/Galileo/Beidou Satellite Receiver");my_OPfunc(OPstring, p_myIRQ);
    ...
    *((unsigned int *)(CH_HARDWARE_BASE + 0 * HARDWARE_OFFSET + WR_NCO3FCW_ACT)) = 0;
	...
	sprintf(OPstring,"noise level %d",SIGMA_2[GPS]);my_OPfunc(OPstring, p_myIRQ);
     initialize_thresholds(50L,GPS);
     initialize_thresholds(50L,GLONASS);
     initialize_thresholds(50L,INMARSAT);    /* JC 30/10/97 INM */
...
/*GB MR says drop this to 7*/
    ...
    //STATE_1_THRES[sys] = (10L*SIGMA_2[sys]);
    STATE_1_THRES[sys] = (10L*SIGMA_2[sys]); // PQ 090714 
...
void return_to_one(int chn, Irq *p_myIRQ)
{
	...
	for( index_debug_sign_store=0;index_debug_sign_store<debug_sign_store_num;index_debug_sign_store++)
	{
		debug_sign_store[index_debug_sign_store] = 0L;
		debug_f_out[index_debug_sign_store] = 0.0;
//		debug_carr_phase_error[index_debug_sign_store] = 0.0;
//		debug_code_phase_error[index_debug_sign_store] = 0.0;
//		debug_carr_filter_state_1_float[index_debug_sign_store] = 0.0;
//		debug_carr_filter_state_2_float[index_debug_sign_store] = 0.0;
 		debug_carrier_NCO_low[index_debug_sign_store] = 0L;
 		debug_code_NCO[index_debug_sign_store] = 0L;
		filtered_sign_store = sign_store_ini;
	}
	index_debug_sign_store = 0;
...
void track_fll_pull_in(int chn, Irq *p_myIRQ)
{
    ...
    // freq_err *= -1; // remove it !!!! If existing, FLL never succeeds.
    ...
    if(task_counter[chn] == FLL_ms_Num)
    {
		...
		{ //code_lock_thres[0][chn]
			sprintf(OPstring,"FLL Pass %dHz %ld>%ld",freq_bin[freq_bin_index],code_lock_thres[CA_primary][chn],STATE_2_THRES[sys[chn]]);my_OPfunc(OPstring, p_myIRQ);
            tracking_state[chn] = STATE_3;
		}
        else
        {
			//sprintf(OPstring,"FLL fail %dHz %ld<%ld",freq_bin[freq_bin_index],code_lock_thres[CA_primary][chn],STATE_2_THRES[sys[chn]]);my_OPfunc(OPstring, p_myIRQ);
            return_to_one(chn, p_myIRQ);
        }
        code_lock_thres[CA_primary][chn] = 0;
        task_counter[chn] = 0;
...
void track_pll_pull_in(int chn, Irq *p_myIRQ)
{
    ...
    if((task_counter[chn] > END_TIME_STATE_3) && (tracking_state[chn] != STATE_1))
    {
        if(filtered_carr_error[L1][chn] < PHASE_LOCK_THRES)
        {
            ...
            task_counter[chn] = 0;
            code_lock_thres[CA_primary][chn] = 0;
			sprintf(OPstring,"PLL OK %f<%f, %ld > %ld",filtered_carr_error[L1][chn],PHASE_LOCK_THRES,code_lock_thres[CA_primary][chn],STATE_3_THRES[sys[chn]]);my_OPfunc(OPstring, p_myIRQ);
        }
		else
		{
...
{   
    ...
    switch(sys[chn])
    {
        ...
        case INMARSAT: /* JC 30/10/97 INM */
            ...
            // code_carr_ratio *= -1; // PQ added in 090818, not yet got a good explanation
#ifdef CLOCK_24_MHZ
/*
            code_carr_ratio *= -1;
*/
...
float carrier_third_order(int chn,float phase_error_deg,float loop_gain, Irq *p_myIRQ)
{                                                                         
    ...
    /* update the hardware with the new frequency */ 
    Set_Carrier_NCO(chn,L1CARRIER,carrier_top[L1][chn],carrier_low[L1][chn], p_myIRQ);
    return (phi_k/ONE_HZ_WORD_CARR); // this return value is only for debug purpose
}
...
void detect_edges(int chn, Irq *p_myIRQ)
{
    ...
    if( (task_counter[chn] % 60) == 0)
    {
		...
		{ // code_lock_thres[CA_primary][chn] is updated in function dot_product_discriminator() earlier.
			sprintf(OPstring,"Detection Edge fail Code %ld<%ld",code_lock_thres[CA_primary][chn],STATE_3_THRES[sys[chn]]);my_OPfunc(OPstring, p_myIRQ);
    ...
    {                                
        ...
        || ((bit_sync[chn] == FOUND_MIDDLE) && (interrupt_number[chn] == 0) ) )
        {
			...
			sprintf(OPstring,"Detect Edge Pass %f<%f",filtered_carr_error[L1][chn],PHASE_LOCK_THRES_HYST);my_OPfunc(OPstring, p_myIRQ);
    ...
    else
    {
		...
		sprintf(OPstring,"Detection Edge fail Phase %f<%f",filtered_carr_error[L1][chn], PHASE_LOCK_THRES_HYST);my_OPfunc(OPstring, p_myIRQ);
        return_to_one(chn, p_myIRQ);
...
void change_integration_period(int chn, Irq *p_myIRQ)
{
    ...
    code_phase_error = dot_product_discriminator(CA_primary,chn,FALSE);
	// code_phase_error *= -1; // PQ remove it!!!!! if added, Code lock lose quickly
    /***********************************************************************/
    /*                                                                     */
    ...
    if(task_counter[chn] < CHANGE_TO_2ms) /* This constant must be even!! */
    {
        ...
        f_out = carrier_third_order(chn,carr_phase_error*360.0F,1.0, p_myIRQ);
    ...
    else
    {
        ...
        switch(task_counter[chn])
        {
            case CHANGE_TO_2ms: // save to bit 23 in debug_sign_store
				sprintf(OPstring,"CHANGE_TO_2ms");my_OPfunc(OPstring, p_myIRQ);
            ...
            case CHANGE_TO_5ms: // save to bit 123 in debug_sign_store
				sprintf(OPstring,"CHANGE_TO_5ms");my_OPfunc(OPstring, p_myIRQ);
            ...
            case CHANGE_TO_10ms:
				sprintf(OPstring,"CHANGE_TO_10ms");my_OPfunc(OPstring, p_myIRQ);
            ...
            case CHANGE_TO_POINT_ONE_10ms:
				sprintf(OPstring,"CHANGE_TO_POINT_ONE_10ms");my_OPfunc(OPstring, p_myIRQ);
                /**************************************************************/
                /*                                                            */
...
*/
        ...
        */
		...
		// f_out is tested with it sign toggle, not working. PQ 20090803
		...
		carrier_low[L1][chn] = freq_offset[chn] + (long)(conv); // 1207389509 <=1207391811 - 2302.571
							 //freq_offset[chn] is only a copy of carrier_low at CHANGE_TO_2ms
		...
		Set_Carrier_NCO(chn,L1CARRIER,carrier_top[L1][chn],carrier_low[L1][chn], p_myIRQ);
// PQ 090803 try using carrier_third_order instead. Failed, 
// 2 digit I output after tens of epochs
//carrier_third_order(chn,carr_phase_error*360.0F,1.0, p_myIRQ);
//carr_filter_state_1_float[L1][chn] += carr_phase_error;
//carr_filter_state_2_float[L1][chn] += carr_filter_state_1_float[L1][chn];
        ...
        // replace code_aid_integrated with code_second_order, keeps tracking till > 300 bits
	...
	// 20 for GPS and 10 for GLONASS, according to current loop closure ms number
	// if loop closure time is 1ms, this condition needs to be checked for 19 times before 1 execution
    if(bit_counter[chn] >= bit_length) 
    {
		...
		debug_sign_store[index_debug_sign_store] = sign_store[chn];
		debug_f_out[index_debug_sign_store] = f_out;
//		debug_carr_phase_error[index_debug_sign_store] = carr_phase_error;
//		debug_code_phase_error[index_debug_sign_store] = code_phase_error;
//		debug_carr_filter_state_1_float[index_debug_sign_store] = carr_filter_state_1_float[L1][chn];
//		debug_carr_filter_state_2_float[index_debug_sign_store] = carr_filter_state_2_float[L1][chn];
 		debug_carrier_NCO_low[index_debug_sign_store] = carrier_low[L1][chn];
 		debug_code_NCO[index_debug_sign_store] = code_freq_word[L1][chn];

	    if (sign_store[chn] > 0)
		{
			filtered_sign_store = (long)(0.99F*(float)filtered_sign_store + 0.01F*(float)sign_store[chn]);
		}
		else
		{ 
			filtered_sign_store = (long)(0.99F*(float)filtered_sign_store - 0.01F*(float)sign_store[chn]);
		}
		if (filtered_sign_store < sign_store_threshold)
		{
			sprintf(OPstring,"lose Code energy in change_integration_period");my_OPfunc(OPstring, p_myIRQ);
	        return_to_one(chn, p_myIRQ);     // Code has unlocked so reacquire
		}


		debug_nav_msg_index ++;
		index_debug_sign_store ++;
		if (index_debug_sign_store == debug_sign_store_num)
		{
			index_debug_sign_store = 0L; // BP for PLL output
		}
		if (debug_nav_msg_index == debug_nav_msg_len)
		{
// 			DISABLEINTERRUPTS;
// 			TermFile File;
// 			if (!File.Open("navData.bin", "w+b"))
// 			{
// 				cio << "nOutput file open error - Program terminating!" << endl;
// 				cio.monitor();
// 			}
// 			File.Write((char*)&debug_nav_msg[0], debug_nav_msg_len); 
// 			File.Close();
			debug_nav_msg_index = 0;
		}
        sign_store[chn] = 0;
		...
		if (sign_store[chn] < 50 && sign_store[chn] > -50)
			sign_store[chn] = sign_store[chn];
        if(sys[chn] == GPS)
        {
...
*/
    ...
    if(tracking_state[chn] == STATE_5F)
    {
		...
		sprintf(OPstring,"enter STATE_6");my_OPfunc(OPstring, p_myIRQ);
...
void change_carrier_filter(float old_T, float new_T, float *carr_phase_error,int chn)
{
	// JPL paper
    ...
    if(old_T == 0.001F)
    {
		// See also JPL Table VII, bottom half(delay 1 update interval).
		// B_L*T=0.01, 3rd order
        K1 = 0.02729F; //PQ modify it from 0.02719F to 0.02729F because JPL paper gives this value
        K2 = 2.562e-4F;
        K3 = 8.084e-7F;
    }
    else
    {
        ...
        /* 15Hz (10ms integration) */
        /*
		// See also JPL Table VII, bottom half(delay 1 update interval). B_L*T=0.15, 3rd order
        K1 = 0.2142F;
        K2 = 0.02208F;
        K3 = 8.655e-4F;
        */
    ...
    phi[2] = (*carr_phase_error)*K3/(T*T*T);
	// WP2112 Eq 100 Phi_n dot dot dot
    phi[1] = ( K3*carr_filter_state_1_float[L1][chn] + K2*(*carr_phase_error) ) / ( T*T );
	// WP2112 Eq 101 Phi_n dot dot
    phi[0] = ( K3*carr_filter_state_2_float[L1][chn] - T*T*phi[1]*0.5F
         - T*T*T*phi[2]/6.0F + K1*(*carr_phase_error)
         + K2*carr_filter_state_1_float[L1][chn])/T;
	// WP2112 Eq 102 Phi_n dot
    ...
    {
        /* 10Hz */
        K1 = 0.1741F; // WP2112 P62 Table VI
        K2 = 0.01313F;
        K3 = 3.585e-4F;
    ...
    *carr_phase_error        = T*T*T*phi[2] / K3;
	// WP2112 Eq 100 Phi_ss
    carr_filter_state_1_float[L1][chn] = (T*T*phi[1] - K2*(*carr_phase_error)) / K3;
    carr_filter_state_2_float[L1][chn] = (T*phi[0] + T*T*phi[1]*0.5F + T*T*T*phi[2]/6.0F
                            - K1*(*carr_phase_error) - K2*carr_filter_state_1_float[L1][chn]) / K3;
}
...
void change_code_filter(float old_T, float new_T, float *phase_error,int chn)
{
    ...
    if(old_T == 0.001F)
    {
		// See also JPL Table VII, bottom half(delay 1 update interval). B_L*T=0.005, 3rd order
        K1 = 0.01406F;
        K2 = 6.698e-5F;
        K3 = 1.067e-7F;    
    }
    ...
    {   
		// See also JPL Table VII, bottom half(delay 1 update interval). B_L*T=0.01, 3rd order
        K1 = 0.02729F;
        K2 = 2.562e-4F;
        K3 = 8.084e-7F;
    }
...
{
    long data_in;    /* local copy of the binary data message, most recent 8 bits */

    data_in = data_store[chn]&255; // Mask off bottom 8 bits, newest 8 bits
    ...
    if(bit_count[chn] == 30)
    {
        ...
        {   
            ...
            if((data_in&1) == 1)
            {
                /* Make the bit corresponding to this chn */
                d30 |= (1 << chn);
            }
            else
            {
			...
			sprintf(OPstring,"Preamble OK, enter STATE_5D. inverted=%ld",inverted);my_OPfunc(OPstring, p_myIRQ);
...
void get_gps_time(int chn)
{
    ...
    {   
        ...
        {                        
			...
			sprintf(OPstring,"GPS t OK, enter STATE_5F");my_OPfunc(OPstring, p_myIRQ);
        ...
        else
        {
        	...
        	sprintf(OPstring,"GPS t fail %d!=%ld",parity_check[chn]&0x3f, (data_in&0x3f));my_OPfunc(OPstring, p_myIRQ);
...
{   
    ...
    switch(sys[chn])
    {
        ...
        case GPS:    
            ...
            // code_carr_ratio *= -1; // PQ added in 090818, not yet got a good explanation, code_aid_integrated
#ifdef CLOCK_24_MHZ
/*
            code_carr_ratio *= -1;
*/
#endif
            // PQ 090804 normally phase lose lock after 24 data bit, now earlier. 
            //code_carr_ratio *= -1; // remove it
            break;
    } // end of switch 

    /* Calculate the estimate of code Doppler from the carrier Doppler */
    aid_freq = ((float)carrier_nominal_freq) * code_carr_ratio; //(Code NCO control word)
    
    if(    (sub_chn != CA_primary)
        && ( (tracking_state[chn] == STATE_7A) || (tracking_state[chn] < STATE_7D && band == L2) )
        && (CODELESS_TRACKING[chn] == FALSE) 
      )   /* Coded operation only - need to check this */
    {
        /* 10Hz loop */
        K1 = 0.1741F;
        K2 = 0.01313F;
...
// PQ 090823 45MHz
K1_X100_XCONV =      K1_X100_XCONV_10Hz;
K2_X100_XCONV =      K2_X100_XCONV_10Hz;
K3_X100_XCONV =      K3_X100_XCONV_10Hz;
...
//        K3_X100_XCONV = 6.15898310246400F;
		// K1_X100_XCONV = K1 * ONE_HZ_WORD_CODE / T=0.1741*171.79869184/0.01=2991.0152249344
#endif
...
*/
    ...
    else // this is the code for change_to_2ms operation
    {   
        /* 1Hz loop */
        K1 = 0.02729F;
        K2 = 2.562e-4F;
...
#ifdef CLOCK_25_AND_20_MHZ        
//PQ 090823 45MHz
K1_X100_XCONV =       K1_X100_XCONV_1Hz;
K2_X100_XCONV =       K2_X100_XCONV_1Hz;
K3_X100_XCONV =       K3_X100_XCONV_1Hz;
...
//        K3_X100_XCONV = 0.01388820624835F;
		// K1_X100_XCONV = K1 * ONE_HZ_WORD_CODE / T=0.02729*171.79869184/0.01=468.83863003136
...
*/
    ...
    if(sys[chn] == GPS)
        ...
        TempState = STATE_7F;
	// change_integration_period == state 5
    if(tracking_state[chn] <= TempState) // code called when running function change_integration_period
    {       
        /* Execute filter */
        //phi_k  = (K1*phase_error + K2*code_filter_state_1[sub_chn][chn] + K3*code_filter_state_2[sub_chn][chn]); // remove it, lose track immediately.
		...
		// PQ 090805 sign of phi_k should be correct, since toggle sign, lose lock from first 20 instead of 100
    }           
    else           
    {                                      
        /********************************************************************/
    ...
    /* Note:- to avoid numerical truncation the integer & decimal     */
    /*        parts of the nominal code frequency have been separated.*/
	//             code_filter_sum is only updated once in this function, a few lines before here.
    conv = phi_k + code_filter_sum[sub_chn][chn] + decimal_code_freq + aid_freq; // (Code NCO control word)
		// Code Loop filter output + ??? + fractional part of 1.023M Code rate control word + carrier aided value
    if(conv < 0.0F)
        conv -= 0.5F;
    else
        conv += 0.5F;
...
//PQ 090823 45MHz
K1_X100_XWORD =     K1_X100_XWORD_10Hz;
K2_X100_XWORD =     K2_X100_XWORD_10Hz;
K3_X100_XWORD =     K3_X100_XWORD_10Hz;
...
{                         
    ...
    if( ((sys_time[chn] + 1)%6 == 0) && ((task_counter[chn]%100) == 99))
    {
        ...
        Set_Flags(chn,PRIMARY,TIM1ARM, p_myIRQ);
        Clear_Flags(chn,PRIMARY,TIM1ARM, p_myIRQ);
		sprintf(OPstring,"enter STATE_7");my_OPfunc(OPstring, p_myIRQ);
...
void P_code_tracking(int chn, Irq *p_myIRQ)
{
        ...
        || ((CODELESS_TRACKING[chn] == TRUE ) && (tracking_state[chn] >= STATE_7A)) )
    {
        ...
        else
        {
			...
			sprintf(OPstring,"exit 1");my_OPfunc(OPstring, p_myIRQ);
			Sleep(30000);
			exit(1);
            /*track_P_L1(chn);
    ...
    else
    {
		...
		{
			// PQ: since P Code is always P(Y) Code nowadays, comment out the whole section.
			sprintf(OPstring,"Nav Data decoding error, enter P code tracking wrongly, exit 2");my_OPfunc(OPstring, p_myIRQ);
			Sleep(30000);
			exit(2);
			/*
...
/* Uncommented JC 4/1/96 */
	...
	{
		// PQ: since P Code is always P(Y) Code nowadays, comment out the whole section.
		sprintf(OPstring,"exit 3");my_OPfunc(OPstring, p_myIRQ);
		Sleep(30000);
		exit(3);
...
/*    if(p_search_int_count[chn] == 400)*/
    ...
    if( (p_search_int_count[chn] == 400) && (tracking_state[chn] < STATE_7E) )
    {
        ...
        sprintf(OPstring,"enter STATE_7E from p_codeless");my_OPfunc(OPstring, p_myIRQ);
    ...
    if(p_search_int_count[chn]%10 == 0)
    {
        ...
        if(p_search_int_count[chn] > 10L)
        {
            if(denom > (500L*SIGMA_2[sys[chn]]))  /* Tested at 300 JC 2/4/96 */
            {
				...
				sprintf(OPstring,"enter STATE_7A from acquire_PW_codeless");my_OPfunc(OPstring, p_myIRQ);
    ...
    if(tracking_state[chn] < STATE_7F) // STATE_7F is the final stage
    {
        if(tracking_state[chn] == STATE_7E)
        {
            if(filtered_carr_error[L2][chn] < PHASE_LOCK_THRES_CODELESS)
            {
                ...
                tracking_state[chn] = STATE_7F;
				sprintf(OPstring,"STATE_7F, L2 carrier has locked");my_OPfunc(OPstring, p_myIRQ);
...
*/
			...
			sprintf(OPstring,"Phi 6");my_OPfunc(OPstring, p_myIRQ);
...
*/
                reinitialize_p_codeless(chn, p_myIRQ);
				sprintf(OPstring,"Code 6");my_OPfunc(OPstring, p_myIRQ);
				tracking_state[chn] = STATE_6;/*GB changed from STATE_7 to STATE_6 to make compatible with 98 f/w*/
            }
        }
